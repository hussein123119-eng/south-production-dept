      ...
      try {
        ...
        if (isLocalEnv) {
          ...
          localTestUsers.forEach(tu => {
            if (!db.users.some(u => u && u.email && u.email.toLowerCase() === tu.email.toLowerCase())) {
              db.users.push(tu);
              modified = true;
            }
          });
